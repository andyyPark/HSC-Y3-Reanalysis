...
def wigner_d(m1, m2, theta, l, l_use_bessel=1.0e4):
    """
...
def wigner_d_parallel(m1, m2, theta, l, ncpu=None, l_use_bessel=1.0e4):
    """
    ...
    p = Pool(ncpu)
    d_mat = np.array(
        p.map(partial(wigner_d, m1, m2, theta, l_use_bessel=l_use_bessel), l)
    )
    p.close()
    p.join()
    return d_mat[:, :, 0].T
...
def wig3j_recur(j1, j2, m1, m2, m3, j3_outmax=None):
    ...
    if (
        j3_min == 0
    ):  # in this case the recursion as implemented doesnot work (all zeros). use sympy function.
        wig_out[j3_min] = wigner_3j(j1, j2, j3_min, m1, m2, m3)
        wig_out[j3_min + 1] = wigner_3j(
            j1, j2, j3_min + 1, m1, m2, m3
        )  # not strictly needed when j3_min>0
...
def Wigner3j(
    m_1, m_2, m_3, j_1, j_2, j_3
):  # Failed attempt to convert sympy function into numpy. Doesnot work at large ell because of numerical errors
    ...
    *Has problems due to rounding errors when numbers get large.*
    ...
    x0 = np.logical_not(
        np.any(
            [
                j_1 + j_2 - j_3 < 0,  # triangle inequalities
                j_1 - j_2 + j_3 < 0,
                -j_1 + j_2 + j_3 < 0,
    ...
    log_argsqrt += (
        ...
        + log_factorial((-j_1 + j_2 + j_3)[x0])
        - log_factorial((j_1 + j_2 + j_3)[x0] + 1)
    )
    ...
    iis = np.arange(
        np.amin(imin), np.amax(imax) + 1
    )  # no need to use x0 here. Can also lead to somewhat wrong answers
    ...
    for i in np.arange(len(iis)):
        ...
        sumres_ii = (
            np.exp(sumres_t - log_den) * sgns[i]
        )  # FIXME: This has numerical issues.
    ...
    return sparse.COO(
        np.where(x0), data=sumres_t * prefid[x0]
    )  # ressqrt taken inside sumres calc
...
def Wigner3j_parallel(m_1, m_2, m_3, j_1, j_2, j_3, ncpu=None, asym_fact=np.inf):
    ...
    c = np.array(np.meshgrid(j_1, j_2, j_3, indexing="ij")).T.reshape(
        -1, 3
    )  # only needed to put cuts below. Otherwise Comb is better
    ...
    if np.all(marr == 0) or np.all(x2):
        ...
        if not np.all(x2):
            ...
            p = Pool(ncpu)
            d_mat2 = p.map(
                partial(wigner_3j_3, asym_fact, m_1, m_2, m_3), c, chunksize=100
            )
            p.close()
            p.join()
            indx1 = np.searchsorted(j_1, c[:, 0])
...
def wigner_3j_3(asym_fact, m1, m2, m3, js):
    ...
    """
    if (
        np.all(np.array(js) > np.absolute([m1, m2, m3]) * asym_fact)
        and np.sum(js) % 2 == 0
    ):
        return np.float32(wigner_3j_000(js[0], js[1], js[2], m1, m2, m3))
    return np.float32(
        wigner_3j(js[0], js[1], js[2], m1, m2, m3)
    )  # .evalf() #this is calling sympy function not Wigner3j
...
):  # assume j1,j2>>j3... not very accurate.. only seems to work when j1==j2
    sj = j_1 + j_2 + 1
