...
home = "/hildafs/projects/phy200017p/andypark/HSC_Y3_reanalysis/skylens/wigner/"
fname = home + f"wig3j_l{lmax}_w{wlmax}_{abs(m2)}_reorder.zarr"
...
def wig3j_recur_2d(j1b, j2b, m1, m2, m3, j3_outmax, step, z1_out):
    ...
    pool = Pool(ncpu)
    out_ij = pool.map(funct, j1s, chunksize=int(step / 40))
    pool.close()
    t2 = time.time()
    ...
    z1[:, j2b : j2b + step, j1b : j1b + step] += out.transpose(
        0, 2, 1
    )  # exploit j1-j2 symmetry
    t3 = time.time()
...
for lb1 in lb:
    ...
    for lb2 in lb:
        if lb2 < lb1:  # we exploit j1-j2 symmetry and hence only compute for j2>=j1
            continue
        if (
            np.absolute(lb2 - lb1 - l_step - 1) > wlmax
        ):  # given j1-j2, there is a min j3 for non-zero values. If it falls outside the required j3 range, nothing to compute.
            continue
